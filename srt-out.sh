...
#  2) camN_prev             : light H264 + OPUS audio, for the control-room wall + audio meter
CAM="$1"
...
# control-room preview: browsers need H264 + Opus (Opus also lets us draw the audio meter)
/usr/bin/ffmpeg -hide_banner -loglevel error -rtsp_transport tcp -i "$SRC" \
  -c:v libx264 -preset ultrafast -tune zerolatency -pix_fmt yuv420p \
  -vf scale=854:-2 -b:v 1200k -g 60 \
